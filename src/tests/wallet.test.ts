...
describe("Wallet provider", () => {
    let walletProvider: EdwinWallet;
    let pk: `0x${string}`;
    ...
    beforeAll(() => {
        ...
        chainNames.forEach(
            (chain) =>
                (customChains[chain] = EdwinWallet.genChainFromName(chain))
        );
    });
    ...
    describe("Constructor", () => {
        it("sets address", () => {
            ...
            walletProvider = new EdwinWallet(pk);
        ...
        it("sets custom chains", () => {
            walletProvider = new EdwinWallet(
                pk,
                customChains
            );
    ...
    describe("Chain", () => {
        ...
        it("generates chains from chain name", () => {
            ...
            const chain: Chain = EdwinWallet.genChainFromName(chainName);
        ...
        it("generates chains from chain name with custom rpc url", () => {
            ...
            const chain: Chain = EdwinWallet.genChainFromName(
                chainName,
                customRpcUrl
            );
        ...
        it("adds chain", () => {
            ...
            const base = EdwinWallet.genChainFromName("base");
        ...
        it("throws if unsupported chain name", () => {
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            expect(() => EdwinWallet.genChainFromName("ethereum")).toThrow();
        });
        it("throws if invalid chain name", () => {
            // intentionally set incorrect chain, ts will complain
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            expect(() => EdwinWallet.genChainFromName("eth")).toThrow();
        });
    });
});
